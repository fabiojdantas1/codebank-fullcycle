...
import (
	...
	"github.com/fabiojdantas1/codebank-fullcycle/domain"
)
...
func (t *TransactionRepositoryDb) SaveTransaction(transaction domain.Transaction, creditCard domain.CreditCard) error {
	statement, err := t.db.Prepare(`insert into transaction (id, credit_card_id, amount, status, description, store, created_at)
...
func (t *TransactionRepositoryDb) CreateCreditCard(creditCard domain.CreditCard) error {
	statement, err := t.db.Prepare(`insert into credit_cards (id, name, number, expiration_month, expiration_year, CVV, balance, balance_limit)
